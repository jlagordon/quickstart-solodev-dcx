...
EOF
  kubectl --kubeconfig $KUBECONFIG create -f $TEMP_DIR/environment_check.yaml
}

cleanup() {
  echo "cleaning up..."
  kubectl --kubeconfig $KUBECONFIG delete -f $TEMP_DIR/environment_check.yaml
  rm -rf $TEMP_DIR
...
wait_ds_ready() {
  while true; do
    local ds=$(kubectl --kubeconfig ${KUBECONFIG} get ds/longhorn-environment-check -o json)
...
validate_ds() {
  ...
  local pods=$(kubectl --kubeconfig ${KUBECONFIG} -l app=longhorn-environment-check get po -o json)
